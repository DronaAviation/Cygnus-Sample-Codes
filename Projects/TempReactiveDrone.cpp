...
void onLoopStart() 
{
    ...
    LED.set(BLUE, ON);
    initemp = Barometer.get(TEMPERATURE) / 100; /* Get initial temperature when you turn ON the developer mode */
...
void plutoLoop() 
{
    ...
    currtemp = Barometer.get(TEMPERATURE) / 100; /* Get current temperature */
    
    Graph.red(initemp, 1);
    Graph.green(currtemp, 1);
    Monitor.println("Currenttemp", currtemp);
    
    /* Check the relation between initial and current temperature and turn on LED for indicating different conditions */
    if (currtemp > initemp) 
    {
        LED.set(RED, ON);
        LED.set(BLUE, OFF);
